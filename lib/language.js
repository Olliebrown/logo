...
function LogoInterpreter (turtle) {
    ...
    function to_arity (func, arity) {
        ...
        /*jslint evil: true */
        var f = eval('f = (function immed' + func.name + '(' + parms.join(',') + ') { return func.apply(this, arguments); })');
        /*jslint evil: false */
        return f;
    }
    ...
    function parse (string) {
        if (string === (void 0) || turtle.limitReached()) {
            return;
        }
        ...
        while (!turtle.limitReached() && string !== (void 0) && string !== '') {
            var atom;
    ...
    function tostr(atom) {
      ...
      if (Type(atom) === 'number') { return atom.toString(); }
      if (Type(atom) === 'word') { return atom; }
      if (Type(atom) === 'list') { return '[' + atom.join(' ') + ']'; }
    ...
    self.routines["to"] = function(list) {
        ...
        while (list.length) {
            var atom = list.shift();
            if (Type(atom) === 'word' && atom.toLowerCase() === 'end') {
                break;
    ...
    self.routines["queue"] = function(stackname, thing) {
      ...
      self.routines["print"] = self.routines["pr"] = function(thing) {
        if (arguments.length > 1) {
          var list = Array.prototype.map.call(arguments, tostr)
          turtle.print(list.join(" ") + "\n")
        } else {
          turtle.print(tostr(thing) + "\n")
        }
      };
    ...
    self.routines["not"] = function(a) {
      ...
      self.routines["setpencolor"] = self.routines["setpc"] = self.routines["setcolor"] = function(a) {
        if (arguments.length === 3) {
          ...
          turtle.setpencolor('#' + rr + gg + bb);
      } else {
          turtle.setpencolor(sexpr(a));
      }
    };
      ...
      self.routines["setbackground"] = self.routines["setscreencolor"] = self.routines["setsc"] = function(a) {
        if (arguments.length === 3) {
          var r = Math.round(aexpr(arguments[0]) * 255 / 99);
          var g = Math.round(aexpr(arguments[1]) * 255 / 99);
          var b = Math.round(aexpr(arguments[2]) * 255 / 99);
          ...
          var gg = (g < 16 ? "0" : "") + g.toString(16);
          var bb = (b < 16 ? "0" : "") + b.toString(16);
          turtle.setscreencolor('#' + rr + gg + bb);
        } else {
          turtle.setscreencolor(sexpr(a));
        }
      };
    ...
    self.routines["repeat"] = function(count, statements) {
        ...
        for (var i = 1; i <= count; ++i) {
            ...
            if (self.turtle.limitReached()) { return }
